...
namespace Estacionamento.View.Gestao
{
    public partial class GerCliente : Form
    {
        ...
        CadDependente cadDep = new CadDependente();
        ...
        public GerCliente()
        {
            ...
            cbConsultar.Items.Add("CONSULTAR PELO LOGIN");
        ...
        private void exibirCliente()
        {
            ...
            else if (cbConsultar.SelectedIndex == 1)
            {
                ...
                {
                    MessageBox.Show("O login digitado não corresponde a nenhum cliente cadastrado!");
                }
            }
            else
            {
        ...
        {
            if (MessageBox.Show("Tem certeza que deseja apagar estes dados? \n Todos os dependentes e veículos ligados a esse cliente também serão apagados!", "Excluir veículo", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                if (gerSitPag.apagarSituacaoPagamentoPorIdCliente(cli.idCliente) &&
                    gerVei.apagarVeiculo(cbPlaca.Text) &&
                    gerVei.apagarVeiculoPorIdCliente(cli.idCliente))
                {
        ...
        private void apagarVeiculo()
        {
            ...
            {
                if (gerSitPag.apagarSituacaoPagamento(cli.idCliente, vei.idVeiculo) &&
                    gerVei.apagarVeiculo(cbPlaca.Text.Trim().ToUpper()))
                {
                    MessageBox.Show("Dados excluídos com sucesso!");
                    limparCamposVeiculo();
                }
        ...
        private void alterarPagamento()
        {
            ...
            if ((val.retornarTextoSemId(cbTipoPlano.Text.Trim()) == "ROTATIVO") || (val.retornarTextoSemId(cbTipoPlano.Text.Trim()) == "DIÁRIO"))
            {
                ...
                vencimento = new DateTime(1900, 1, 1);
            }
            else
